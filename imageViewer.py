...
class ImageViewer(Frame):
    ...
    def __init__(self, master=None):
        Frame.__init__(self, master=master, width=600, height=400)
    ...
    def show_image(self, img=None):
        ...
        if self.master.is_spectral is True:
    ...
    def start_draw(self, event):
        self.x = event.x
        self.y = event.y
        # self.draw_coord.append((self.x, self.y))

    def draw(self, event):
        # rotation_count = self.master.operation.count('retation') % 4 
        # self.draw_coord.append((event.x, event.y))
        self.draw_ids.append(self.canvas.create_line(self.x, self.y, event.x, event.y, width=2,
                                                    fill="#{:02x}{:02x}{:02x}".format(draw_color['r'],draw_color['g'],draw_color['b']), capstyle=ROUND, smooth=True))
        ...
        cv2.line(self.master.processed_image, (int(self.x * self.ratio), int(self.y * self.ratio)),
                ...
                (draw_color['b'], draw_color['g'], draw_color['r']), thickness=int(self.ratio * 2),
                lineType=8)
        self.x = event.x
        self.y = event.y
    ...
    def end_crop(self, event):
        ...
        self.master.operation.append((start_x, end_x, start_y, end_y))
